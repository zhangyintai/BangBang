...
% alpha0 = (sin((linspace(1/(2 * P), (2 * P - 1)/(2 * P), P)+0.5) * pi) + 1) * 0.5 * tau;
...
objective = @(x)test.BangBang_Ising_(H1, H, 1, ntu_tol, svd_tol, 1e-9, Etol, ctm_svd_tol, P, 0, [x(1:2:2*P), x(2:2:2*P)], 1, 0);
objective_test = @(x)test.BangBang_Ising_(H1, H, 1, ntu_tol, svd_tol, 1e-9, Etol, ctm_svd_tol, P, 1, [x(1:2:2*P), x(2:2:2*P)], 2, 0);
objective_tangentmps = @(x)test.BangBang_Ising_(H1, H, 1, ntu_tol, svd_tol, 1e-9, Etol, ctm_svd_tol, P, 0, [x(1:2:2*P), x(2:2:2*P)], 2, 0);
...
% ub = [0.65, 0.25, 0.65, 0.25, 0.65, 0.25, 0.65, 0.25, 0.65, 0.25, 0.65, 0.25, ];
ub = [1, 1, 1, 1, 1, 1, 0.25, 0.25, 0.125, 0.125] * pi;
