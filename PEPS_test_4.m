classdef PEPS_test_4 < matlab.mixin.Copyable
    properties
        a = 0;
        b = 0;
        d = 2;
    ...
    methods
        ...
        function ApplyGate(obj, gate_a, gate_b, bond_dir, method, ntu_tol, svd_tol, pinv_tol)
        ...
        function [energy, rho] = BangBang_Ising_(obj, H1, it_rt, ntu_tol, svd_tol, pinv_tol, ctm_tol, ctm_svd_tol, P, change_peps, x, ctm, rand_gauge)
            ...
            elseif ctm == 2
                ...
                [rho, TA1, TB1, TA3, TB3] = obj_temp.VUMPStest(vumps_tol, vumps_e_tol, ((kron(H1, eye(obj.d)) + kron(eye(obj.d), H1)) / 4 - ...
                    kron([[1, 0]; [0, -1]], [[1, 0]; [0, -1]])), min(2 * P, 25));
                obj_temp.TA{1} = TA1;
                obj_temp.TA{3} = TA3;
                obj_temp.TB{1} = TB1;
                obj_temp.TB{3} = TB3;
                disp(rho)
                energy = trace(((kron(H1, eye(obj.d)) + kron(eye(obj.d), H1)) / 4 - ...
                    kron([[1, 0]; [0, -1]], [[1, 0]; [0, -1]])) * rho)
                energy = real(energy);
                fprintf("%.9e\n", energy);
                disp(real(trace(rho * (kron([[0, 1]; [1, 0]], eye(obj.d)) + kron(eye(obj.d), [[0, 1]; [1, 0]])) / 2)))
